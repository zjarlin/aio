...
fn api_status_for_message(message: &str) -> StatusCode {
    ...
    if normalized.starts_with("not implemented") || message.contains("尚未实现") {
...
mod tests {
    ...
    fn missing_database_url_maps_to_service_unavailable() {
        let error = ApiError::from(anyhow::anyhow!("missing program database url"));
    ...
    #[test]
    fn unimplemented_service_maps_to_not_implemented() {
        let error = ApiError::from(anyhow::anyhow!("配置中心状态尚未实现"));
