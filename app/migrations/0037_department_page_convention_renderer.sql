CREATE FUNCTION engine_use_department_convention_renderer(definition JSONB)
RETURNS JSONB
LANGUAGE SQL
IMMUTABLE
AS $$
    SELECT jsonb_set(
        ...
        COALESCE(
            (
                SELECT jsonb_agg(
                    CASE
                        WHEN page.value ->> 'name' IN ('departments', 'department')
                            OR page.value ->> 'title' IN ('组织部门', '部门管理')
...
SET definition = engine_use_department_convention_renderer(definition),
    version = version + 1,
...
SET definition = engine_use_department_convention_renderer(definition),
    content_hash = 'migrated-department-convention:' || md5(
        engine_use_department_convention_renderer(definition)::TEXT
    )
WHERE definition ->> 'name' = 'aio-first-party';
...
DROP FUNCTION engine_use_department_convention_renderer(JSONB);
