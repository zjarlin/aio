CREATE FUNCTION engine_program_v10(definition JSONB)
RETURNS JSONB
LANGUAGE SQL
IMMUTABLE
AS $$
                        ...
                        page.value || jsonb_build_object(
                            ...
                            COALESCE(
                                (
                                    SELECT jsonb_agg(
                                        CASE
                                            WHEN COALESCE(endpoint.value ->> 'title', '') = ''
                                            THEN endpoint.value - 'name' - 'intent' - 'title'
                                            ELSE endpoint.value - 'name' - 'intent'
                                        END
...
SET definition = engine_program_v10(definition)
WHERE COALESCE((definition ->> 'schema_version')::INTEGER, 0) < 10;

DROP TRIGGER engine_program_revisions_immutable ON engine_program_revisions;

UPDATE engine_program_revisions
SET definition = engine_program_v10(definition),
    content_hash = 'migrated-v10:' || md5(engine_program_v10(definition)::TEXT)
WHERE COALESCE((definition ->> 'schema_version')::INTEGER, 0) < 10;
...
DROP FUNCTION engine_program_v10(JSONB);
