CREATE FUNCTION engine_use_user_convention_renderer(definition JSONB)
RETURNS JSONB
LANGUAGE SQL
IMMUTABLE
AS $$
    SELECT jsonb_set(
        ...
        COALESCE(
            (
                SELECT jsonb_agg(
                    CASE
                        WHEN page.value ->> 'name' IN ('users', 'user')
...
SET definition = engine_use_user_convention_renderer(definition),
    version = version + 1,
...
SET definition = engine_use_user_convention_renderer(definition),
    content_hash = 'migrated-user-convention:' || md5(
        engine_use_user_convention_renderer(definition)::TEXT
    )
WHERE definition ->> 'name' = 'aio-first-party';
...
DROP FUNCTION engine_use_user_convention_renderer(JSONB);
