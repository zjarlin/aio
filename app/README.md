...
Studio 宿主不保存业务页面函数。页面函数只随独立应用生成到 `generated/apps/<application-id>/src/pages`。业务接口由 Studio 同步到 `lib/biz/<application-id>`：`src/generated` 保存每个 feature 的契约、Controller、Service 实现起点和骨架所有权清单；人工修改后的 `service_impl.rs` 不再由生成器覆盖。
