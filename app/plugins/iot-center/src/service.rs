...
/// 使用共享 RecordStore 管理物联网低代码模型和记录。
#[derive(Clone)]
...
impl IotService {
    ...
    async fn list_records(&self, model_name: &str) -> Result<Vec<az_plugin_core::DataRecordView>> {
        let page = self
            .store
            .executor()
...
fn prepare_fixture_telemetry(values: &BTreeMap<String, Value>) -> Result<EnvironmentTelemetry> {
    crate::telemetry::process_telemetry(values).context("模拟遥测解码或领域校验失败")
}
...
mod tests {
    ...
    #[test]
    fn template_contains_five_dynamic_models() {
        let names = MODEL_DEFINITIONS
            .iter()
            .map(|definition| definition.name)
            .collect::<Vec<_>>();

        // 关键断言：物联网插件的正式业务模型必须全部进入动态数据引擎。
        assert_eq!(
            names,
            [
                PRODUCT_MODEL,
