...
impl From<OperationRevision> for OperationRevisionView {
    fn from(revision: OperationRevision) -> Self {
        Self {
            id: revision.id,
...
impl EngineStore {
    ...
    pub async fn create_operation(&self, draft: OperationDraft) -> anyhow::Result<OperationBundle> {
        ...
        Ok(OperationBundle {
            ...
            revision: created_revision.into(),
        })
    }
    ...
    async fn execute_and_audit(
        ...
        let execution = tokio::task::spawn_blocking(move || {
            execute_revision(&execution_revision, &execution_context)
        })
        .await
        .context("等待 engine operation 执行任务失败")?;
...
pub fn execute_revision(
    ...
    if revision.executor_kind != EXECUTOR_RHAI {
        bail!("当前不支持 operation executor: {}", revision.executor_kind);
    }
...
fn validate_operation_draft(draft: &OperationDraft) -> anyhow::Result<()> {
    ...
    validate_revision_input(&OperationRevisionInput {
        executor_kind: draft.executor_kind.clone(),
...
fn validate_revision_input(input: &OperationRevisionInput) -> anyhow::Result<()> {
    if input.executor_kind != EXECUTOR_RHAI {
        bail!("首版 operation 只支持 rhai executor");
    }
    if input.source_text.trim().is_empty() {
        bail!("operation source_text 不能为空");
    }
    if input.source_text.len() > MAX_SOURCE_BYTES {
        bail!("operation source_text 不能超过 {MAX_SOURCE_BYTES} 字节");
    }
...
mod tests {
    ...
    #[test]
    fn rejects_unbounded_or_privileged_executor_configuration() {
        let draft = OperationDraft {
            operation_key: "device.power".to_string(),
            display_name: "设备电力".to_string(),
            description: String::new(),
            method: "POST".to_string(),
            executor_kind: "bash".to_string(),
            source_text: "echo unsafe".to_string(),
            input_schema: default_object_schema(),
            output_schema: default_object_schema(),
            capability_policy: default_capability_policy(),
            timeout_ms: DEFAULT_TIMEOUT_MS,
        ...
        // 首版只允许无系统权限的 Rhai，不能把在线代码升级成 shell 执行。
