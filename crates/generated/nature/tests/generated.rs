...
fn fixture_map_decodes_and_validates_the_domain_value() -> anyhow::Result<()> {
    ...
    source.insert("raw_temperature".to_string(), serde_json::json!(253f64));
    source.insert("raw_humidity".to_string(), serde_json::json!(612f64));
...
fn missing_fixture_field_is_rejected() {
    ...
    let message = result
        ...
        .unwrap_or_default();
    assert!(message.contains("raw_temperature"));
}
#[test]
fn fixture_type_error_is_rejected() {
    ...
    source.insert("raw_humidity".to_string(), serde_json::json!(612f64));
    source.insert(
        "raw_temperature".to_string(),
        serde_json::json!("不是有效数值"),
    );
    let result = az_aio_nature_generated::bindings::decode_environment_telemetry(&source);
    assert!(result.is_err());
}
...
fn out_of_range_fixture_value_is_rejected() {
    ...
    source.insert("raw_temperature".to_string(), serde_json::json!(253f64));
    source.insert("raw_humidity".to_string(), serde_json::json!(612f64));
    source.insert("raw_temperature".to_string(), serde_json::json!(1260f64));
    let result = az_aio_nature_generated::bindings::decode_environment_telemetry(&source);
    assert!(result.is_err());
}
