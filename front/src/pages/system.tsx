...
const modules: SystemModule[] = [
    {
        ...
        prompt: "把用户管理模块整理成角色边界、状态流转、命令执行顺序和风险点。",
    ...
    },
    {
        ...
        prompt: "把角色治理整理成权限矩阵、菜单授权覆盖和高风险授权边界。",
        progressiveDisclosure: [
            "先列角色和菜单覆盖面。",
            "再查看单角色绑定明细。",
            "最后执行授权或删除。",
    ...
    },
    {
        ...
        prompt: "把导航管理整理成二维上下文树、菜单可见性和权限码治理。",
        progressiveDisclosure: [
            "先列当前菜单树。",
            "再判断挂载、移动、隐藏还是删除。",
            "最后校验 permission_code 与排序。",
        ],
        cliCommands: [
    ...
    },
    {
        ...
        prompt: "把组织上下文整理成层级关系、命名规范、隔离边界和同步顺序。",
        progressiveDisclosure: [
            "先列层级和父子关系。",
            "再确定新增、改名、迁移还是删除。",
            "最后校验环境隔离与影响面。",
    ...
    },
    {
        ...
        prompt: "把字典与元数据整理成 group/item 结构、枚举约束和发布影响面。",
        progressiveDisclosure: [
            "先列字典组和条目现状。",
            "再判断新增组、改组还是改条目。",
            "最后输出影响的业务字段和发布检查。",
    ...
    },
    {
        ...
        prompt: "把安全边界整理成权限等级、凭证策略、审批链路和审计要求。",
        progressiveDisclosure: [
            "先列实际会触发的高风险动作。",
            "再判断谁能调、如何审计、何时拦截。",
            "最后把可执行 CLI 与危险能力分层。",
        ...
        cliCommands: [
            "aio system docs --module security",
            "aio system docs --module users",
            "aio system docs --module roles",
        ],
        checklist: [
            "梳理脚本权限等级",
            "定义外部调用审批链路",
...
function moduleSnapshot(module: SystemModule, state: ModuleWorkbenchState) {
    ...
    return [
        ...
        `模块提示词：${module.prompt}`,
        "渐进式披露：",
        ...module.progressiveDisclosure.map((item, index) => `${index + 1}. ${item}`),
        "可调用 CLI：",
        ...module.cliCommands.map((command) => `- ${command}`),
...
export default function SystemPage() {
    ...
    function sendModuleToAi(module: SystemModule) {
        ...
        emitAiWorkspaceSeed({
            ...
            prompt: `${module.suggestedPrompt}\n同时结合该模块内置的 CLI 能力和渐进式披露步骤给出执行方案。`,
            open: true,
        });
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            ...
                            <CardTitle className="mt-3 text-3xl tracking-tight">
                                ...
                                这页现在不是静态说明。它同时承担模块提示词、渐进式披露、
                                agent CLI 配方和 AI 整理入口。人可以看，agent 也可以直接照着调用。
                                </CardDescription>
                        </div>
            ...
            <Tabs defaultValue="workbench" className="space-y-4">
                <div className="rounded-3xl border bg-[#faf7ef] p-2">
                    <TabsList className="grid h-auto w-full grid-cols-2 rounded-2xl bg-transparent p-0">
                        <TabsTrigger value="workbench" className="rounded-2xl py-3">
                            治理工作台
                        </TabsTrigger>
                        <TabsTrigger value="settings" className="rounded-2xl py-3">
                            正式配置
                        </TabsTrigger>
                    </TabsList>
                </div>
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-5 p-5">
                                <section className="space-y-3">
                                    <div className="text-sm font-medium">模块提示词</div>
                                    <div className="rounded-2xl border bg-muted/20 px-4 py-4 text-sm leading-7 text-foreground">
                                        {activeModule.prompt}
                                    </div>
                                </section>

                                <section className="space-y-3">
                                    <div className="text-sm font-medium">渐进式披露</div>
                                    <div className="grid gap-2">
                                        {activeModule.progressiveDisclosure.map((item, index) => (
                                            <div
                                                key={item}
                                                className="rounded-2xl border bg-background px-3 py-3 text-sm"
                                            >
                                ...
                                <section className="space-y-3">
                                    <div className="flex items-center justify-between gap-3">
                                        <div className="text-sm font-medium">Agent CLI</div>
                                        <Badge variant="outline" className="rounded-full">
                                            {activeModule.cliCommands.length} 条
                                        </Badge>
                                    </div>
                                    <div className="grid gap-2">
                                        {activeModule.cliCommands.map((command) => (
                                            <div
                                                key={command}
                                                className="rounded-2xl border bg-stone-950 px-3 py-3 font-mono text-xs leading-6 text-stone-100"
                                            >
                                                {command}
                                            </div>
                                        ))}
