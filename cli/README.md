...
应用仓库的 `aio.toml` 是插件来源真源。每个来源只需配置 `git` 和可选 `rev`；`aio plugin sync` 会拉取仓库、读取根目录 `aio-plugin.toml`、分别发现 client/server Cargo 包、更新 feature 依赖并生成 Dill 注册入口。

当前可执行插件是编译期 Rust 源码依赖。安装或卸载后需要重新构建应用，不执行远端仓库提供的脚本，也不加载不安全的动态库。运行时插件唯一性只由具体类型的 `TypeId` 决定；Git 地址、Cargo 包名和页面 id 分别只用于来源、构建和业务导航。
