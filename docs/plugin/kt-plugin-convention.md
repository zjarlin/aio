...
- 共享业务逻辑放在 `commonMain`。
- 需要 AIO 原生页面时，目标是 `wasm-component` 并实现 `aio:plugin/page@1`；组件返回 `PageDefinition` 和事件结果，不直接操作宿主 DOM。
- 独立 Compose Multiplatform 页面只能作为隔离页面运行，不能伪装成 AIO 原生控件树。
- 在 AIO Component ABI 正式发布前，Kotlin/Wasm 页面属于预览目标，不得提交为可自动安装的稳定插件。

## 服务端

纯计算、短请求处理可使用 `wasmWasi`。需要 JDBC、协程常驻任务、第三方 JVM SDK 或不受 WASI 支持的系统能力时，构建可执行 JAR，并在清单中声明 `process`、启动命令和健康检查。宿主负责进程启动、停止、超时和日志，不允许插件自行守护。
